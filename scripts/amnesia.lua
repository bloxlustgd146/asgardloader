local info_t = { username = "asgardleaks", build = "67", discord = 0 }
...
local amnesia = {
	...
	online_count = 0,
	config_selected_name = "cfg by asgardleaks",
...
function amnesia_is_asgardleaks_profile() return false

end

local function amnesia_display_build_str()
	local b = tostring(amnesia.build or "stable")
	if amnesia_is_asgardleaks_profile() and string.lower(b) == "debug" then
		return "developer"
	end
	return b
end
...
menu.info = {
	...
	user = groups.fakelag:label((" \f<silent>%s   \v%s • %s"):format(MENU_INFO_USER_ROLE, amnesia_is_asgardleaks_profile, amnesia_display_build_str())),
	...
	site = groups.fakelag:label(" \f<silent>Leaked by   \vdsc.gg/asgardleaks"),
	...
	config = {
		...
		discord_server = groups.other:button("\vDiscord Server", function()
			...
			if not ok then
				...
				amnesia.print("\aB6B665Copied Discord link to clipboard: \vhttps://dsc.gg/asgardleaks")
			end
		end),
		cloud_dashboard = groups.other:button("\vLeaked by asgardleaks", function()
			local ok = pcall(function()
				panorama.open().SteamOverlayAPI.OpenExternalBrowserURL("https://dsc.gg/asgardleaks")
			end)
			if not ok then
				clipboard.set("https://dsc.gg/asgardleaks")
				amnesia.print("\aB6B665Copied link to clipboard: \vhttps://dsc.gg/asgardleaks")
			end
		end),
		menu.space(groups.other),
	},
...
function amnesia._refresh_asgardleaks_diagnostics()
	-- Diagnostics removed
end
...
menu.loadouts = {
	config = {
		...
		list = groups.angles:listbox("Loadouts", {"cfg by asgardleaks"}),
	    selected = groups.angles:label("Selected: \vcfg by asgardleaks"),
...
menu.stats = {
	...
	loaded = groups.other:label("\f<silent>" .. amnesia.t("Times loaded") .. "\t\v" .. "333"),
	playtime = groups.other:label("\f<silent>" .. amnesia.t("Hours played") .. "\t\v" .. "666"),
	killed = groups.other:label("\f<silent>" .. amnesia.t("Enemies eliminated") .. "\t\v" .. "1337"),
	evaded = groups.other:label("\f<silent>" .. amnesia.t("Missed to you") .. "\t\v" .. "31043"),
}
...
local BUILTIN_CONFIG_NAME = "cfg by asgardleaks"
...
kill_phrases = {
	...
	{"ᅔ>.< член в заднице у пендосов ＡＭＮＥＳＩＡ.ＰＬＵＳᅕஃ", "ᅕ ＡＭＮＥＳＩＡ.ＰＬＵＳ ᅕ"},
    ...
    {"dsc.gg/asgardleaks - 𝖆𝖒𝖓𝖊𝖘𝖎𝖆.𝖈𝖋𝖉 - 𝖏𝖚𝖘𝖙 𝖔𝖕𝖊𝖓 𝖙𝖍𝖊 𝖙𝖎𝖈𝖐𝖊𝖙", "𝖆𝖓𝖉 𝖇𝖚𝖞 𝖋𝖚𝖈𝖐𝖎𝖓𝖌 𝖙𝖍𝖊 𝖇𝖊𝖘𝖙 𝖘𝖈𝖗𝖎𝖕𝖙"},
    ...
    {"DONT PASTE SHIT DOGS", "my dog is a golden, ur dog is a homeless"}
},
...
kill_phrases_ru = {
    {"☆꧁✬ ᴀᴍɴᴇsɪᴀ.ᴘʟᴜs ✬꧂☆", "☆꧁✬ ᴜɴsᴛᴏᴘᴘᴀʙʟᴇ ✬꧂☆"},
    {"ваше сало хуй сосало", "поросенок пидор был"},
    {"соси хуй чмо", "лошара"},
    {"♕ ʀᴜssɪᴀɴ ᴘᴏʟɪᴄᴇ ♕", "♕ M͜͡V͜͡D͜͡ x͜͡ O͜͡V͜͡D͜͡ ♕"},
    {"ᅔ>.< член в заднице у пендосов ＡＭＮＥＳＩＡ.ＰＬＵＳᅕஃ", "ＡＭＮＥＳＩＡ.ＰＬＵＳᅕ"},
    {"амнезия > ты",  "сакнул мои яйца"},
    {"ᴀᴍɴᴇsɪᴀ.ᴘʟᴜs - ᴏꜰꜰɪᴄɪᴀʟ ᴡᴇʙsɪᴛᴇ", "ᴇɴᴛᴇʀ ɪᴛ ᴀɴᴅ ʙᴜʏ ᴛʜᴇ ʙᴇsᴛ ʟᴜᴀ"},
    {"𝐬𝐤𝐞𝐞𝐭 𝐝𝐨𝐧𝐭 𝐧𝐞𝐞𝐝 𝐮𝐩𝐝𝐚𝐭𝐞", "𝐰𝐢𝐭𝐡 𝐚𝐦𝐧𝐞𝐬𝐢𝐚.𝐜𝐟𝐝"},
    {"мамка орала?", "я ей по башке ебанул"},
    {"спать", "узкий бля"},
    {"втопи ебальник свой", "безшейный хуесос"},
    {"выйди на улицу", "толстопузый)"},
    {"ᴛʜɪs ɪs ᴀᴍɴᴇsɪᴀ ᴘᴏᴡᴇʀ", "ʏᴏᴜ ᴄᴀɴ'ᴛ ᴄᴏᴍᴘᴇᴛᴇ"},
    {"у тебя аашки из хуйни какой-то", "у меня из amnesia.plus"},
    {"ａｍｎｅｓｉａ  ｍａｋｅｓ ｍｅ  ｓｔｒｏｎｇ", "ᴀᴍɴᴇsɪᴀ.ᴘʟᴜs ᴘʀᴇᴍɪᴜᴍ ʟᴜᴀ"},
    {"ᴅ𝗌ᴄ.ɢɢ/ɢᴀᴍᴇ𝗌ᴇɴ𝗌ɪᴄᴀʟ", "𝟧𝟢𝟢 𝗋𝗎𝖻𝖼𝗁𝗂𝗄𝗈𝗏 𝗓𝖺 𝗌𝖼𝗋𝗂𝗉𝗍"},
    {"ᴀᴍɴᴇsɪᴀ.ᴘʟᴜs ᴍᴀᴋᴇs ᴍᴇ ᴊᴇsᴜs", "ᴡᴇʙsɪᴛᴇ. -> ᴀᴍɴᴇsɪᴀ.ᴘʟᴜs "},
    {"твоя мама просила не унижать", "но я не читаю правила"},
    {"не говори мне ничего", "нищий скунс"},
    {"с таким конфигом", "тебе бы в столовке убирать тарелки"},
    {"нажми F10+ENTER", "пускай комп отдохнёт от позора"},
    {"я твою мать пиздил", "битой с гвоздями)"},
    {"ｋｉｔｔｙｈｏｏｋ? ｎｏ ａｍｎｅｓｉａ.ｐｌｕｓ", "ʜᴇᴀᴅsʜᴏᴛ ᴍᴀᴄʜɪɴᴇ ᴡɪᴛʜ ᴀᴍɴᴇsɪᴀ.ᴘʟᴜs"},
    {"если это твой лучший раунд", "то лучше не показывай худший"},
    {"ты не тиммейт, ты контент", "для хайлайтов amnesia.cfd"},
    {"твоя мама ебала меня", "своей пиздой", "хехеххехе"},
    {"с таким скиллом", "тебе даже amnesia.cfd не поможет"},
    {"♕ skeet.cc ♕", "♕ amnesia.plus ♕"},
    {"ещё один герой паблика", "без мамки и без норм луа"},
    {"коннект кидай шлюха", " пойду унижу тебя 1х1"},
},
...
do
	...
	if type(s) ~= "table" then
		...
		s.user = "dsc.gg/asgardleaks"
	end
	_AMNESIA_SESSION = nil
	
	-- BYPASS: Всегда считаем пользователя автором (estk)
	local is_author = true -- (type(s) == "table" and (s.user == "dsc.gg/asgardleaks" or s.user == "asgardleaks"))
	...
	amnesia.user.name = "asgardleaks" -- BYPASS: Всегда устанавливаем как estk
	...
	pcall(amnesia._refresh_asgardleaks_diagnostics)
	...
	client.delay_call(AUTH_GREET_DELAY_SEC, function()
		if logger and logger.invent then
			...
			local greet = night and "Good night, asgardleaks, loser -> " or "Good day, asgardleaks, loser -> "
			logger.invent("auth", {
	...
	local function _apply_public_config_from_stats(data)
		...
		if amnesia_is_asgardleaks_profile() then
			pcall(amnesia._refresh_asgardleaks_diagnostics)
		end
	end
	...
	local function _asgardleaks_pull_user_stats()
		if not amnesia_asgardleaks_profile() then
			return
		end
		local sid = tostring(amnesia.steamid64 or "")
		local st = tostring(amnesia.server_token or "")
	...
	pcall(function()
		...
		client.set_event_callback("round_start", function()
			...
			if online_count > 0 and logger and logger.invent and vars.misc.logs.on.value and vars.misc.logs.events:get("Shared users") then
				...
				if amnesia_is_asgardleaks_profile() and (_now_pull - _last_pull) > 50 then
					amnesia._diag_user_stats_last = _now_pull
					_asgardleaks_pull_user_stats()
				end
			end)
			end)
		end)
